...
const LineChart = memo(({ area = false, datasets = [], height = 300, labels = [], legend = true, stepped = false, title = "", isCurrency = false }) => {
  ...
  const enriched = useMemo(() =>
    enrichDatasets(datasets, theme).map((ds, i) => ({
      ...ds,
      fill: area
        ? { above: defaultColorsAlpha(theme, 0.08)[i % 6], below: defaultColorsAlpha(theme, 0.02)[i % 6], target: "origin" }
        : false,
      stepped,
      ...(area && { pointRadius: 0, pointBorderWidth: 0 }),
    })),
    [datasets, theme, area, stepped]
  );
  ...
  return (
    <Box sx={{ height, position: "relative", width: "100%" }}>
      <LineChartJs ref={chartRef} data={{ datasets: enriched, labels }} options={options} redraw />
    </Box>
  );
});
