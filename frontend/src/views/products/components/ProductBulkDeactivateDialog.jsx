...
import { useDeactivateProductsMutation } from "@views/products/hooks";
import { showNotification } from "@store/notifications/notificationsSlice.js";

const ProductBulkDeactivateDialog = ({ selectedIds, selectedCount, onClose, onClearSelection, open }) => {
  const dispatch = useDispatch();

  const mutation = useDeactivateProductsMutation({
    onSuccess: (data) => {
      const { summary } = data;
      let message = `${summary.deactivated} produk berhasil dinonaktifkan`;
    ...
    <Dialog fullWidth maxWidth="xs" onClose={mutation.isPending ? undefined : onClose} open={open}>
      ...
      <DialogContent sx={{ pt: 2.5, pb: 3 }}>
        ...
        <Typography variant="body2" color="warning.main" sx={{ mt: 2 }}>
          Produk yang dinonaktifkan tidak akan muncul di daftar produk aktif dan tidak dapat digunakan dalam transaksi.
        </Typography>
      </DialogContent>
      ...
      <DialogActions sx={{ px: 3, py: 2.5 }}>
        ...
        >
          {mutation.isPending ? "Menonaktifkan..." : `Nonaktifkan ${selectedCount} Produk`}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
export default ProductBulkDeactivateDialog;