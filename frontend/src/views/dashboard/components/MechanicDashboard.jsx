...
const EmptyMechanicSVG = () => (
  <Box component="svg" viewBox="0 0 200 120" sx={{ width: { xs: 100, sm: 120 }, height: { xs: 60, sm: 72 }, opacity: 0.15, color: "text.secondary" }}>
...
const EmptyState = ({ title, description, children }) => {
  ...
  return (
    <Stack sx={{ alignItems: "center", justifyContent: "center", py: { xs: 4, sm: 6, md: 8 }, px: { xs: 2, sm: 0 }, gap: { xs: 2, sm: 3 }, textAlign: "center", flexGrow: 1, minHeight: { xs: 200, sm: 260, md: 300 } }}>
      {children}
  ...
  return (
    <Card sx={{ borderRadius: br }}>
      <Box sx={{ p: { xs: 2, sm: 2.5, md: 3 }, pb: 2 }}>
        ...
        <Skeleton variant="text" width={isMobile ? 180 : 220} height={16} sx={{ mt: 0.5, minWidth: 120 }} />
      </Box>
      <Divider />
      <Box sx={{ p: { xs: 2, sm: 2.5, md: 3 }, display: "flex", flexDirection: { xs: "column", md: "row" }, alignItems: "center", gap: { xs: 3, md: 4 } }}>
        <Box sx={{ flex: 1, display: "flex", justifyContent: "center" }}>
          <Skeleton variant="circular" width={isMobile ? 160 : 220} height={isMobile ? 160 : 220} />
        </Box>
        <Stack sx={{ gap: { xs: 1.5, sm: 2 }, minWidth: { xs: "100%", md: 200 }, maxWidth: { xs: "100%", md: 280 }, width: "100%" }}>
          {[1, 2, 3].map((i) => (<Skeleton key={i} variant="rounded" width="100%" height={isMobile ? 48 : 56} sx={{ borderRadius: br }} />))}
        </Stack>
      </Box>
    </Card>
  );
...
const TableSkeleton = () => {
  ...
  return (
    <Card sx={{ borderRadius: br }}>
      ...
      <Divider />
      <Box sx={{ p: { xs: 2, sm: 2.5, md: 3 } }}>
        <Skeleton variant="rounded" width="100%" height={isMobile ? 250 : 300} sx={{ borderRadius: br }} />
      </Box>
    </Card>
  );
};
...
const MechanicDashboard = ({ data, isLoading, refetch }) => {
  ...
  const taskChartData = useMemo(() => {
    ...
    return {
      datasets: [{ data: [data.overallStats.completed || 0, data.overallStats.pending || 0], backgroundColor: [theme.palette.secondary.main, alpha(theme.palette.secondary.main, 0.15)], borderWidth: 0 }],
      labels: ["Selesai", "Pending"],
    };
  }, [data, theme]);

  const hasData = useMemo(() => data?.overallStats?.total > 0 || data?.pendingTasks?.length > 0, [data]);

  const tableHeaders = ["Layanan", "No. Order", "Kendaraan", "Status"];

  const renderRow = (task) => [
    <Typography key="service" variant="body2" sx={{ fontWeight: 500 }} noWrap>{task.serviceName}</Typography>,
    <Typography key="order" variant="body2" color="text.secondary" sx={{ fontFamily: "monospace" }} noWrap>{task.orderNumber}</Typography>,
    <Typography key="plate" variant="body2" color="text.secondary" noWrap>{task.plateNumber || "—"}</Typography>,
    <Chip key="status" label={normalizeEnumText(OrderStatus[task.status] || task.status)} color={statusColorMap[task.status] || "default"} size="small" variant="outlined" sx={{ fontWeight: 500, fontSize: "0.75rem", height: 24 }} />,
  ];
  ...
  if (isLoading) {
    return (
      <Stack sx={{ gap: { xs: 2, sm: 3, md: 4 } }}>
        <HeaderSkeleton />
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr 1fr" }, gap: { xs: 2, sm: 2.5, md: 3 } }}>{[1, 2, 3].map((i) => (<SummaryCardSkeleton key={i} />))}</Box>
        <ProgressSkeleton />
        <TableSkeleton />
      </Stack>
    );
  ...
  return (
    <Stack sx={{ gap: { xs: 2, sm: 3, md: 4 } }}>
      ...
      {/* Summary Cards */}
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr 1fr" }, gap: { xs: 2, sm: 2.5, md: 3 } }}>
      ...
      <Card sx={{ borderRadius: `${theme.shape.borderRadius}px` }}>
        <Box sx={{ p: { xs: 2, sm: 2.5, md: 3 }, pb: 2 }}>
          <Typography variant={isMobile ? "subtitle1" : "h6"} color="text.primary" sx={{ fontWeight: 600, letterSpacing: "-0.01em" }}>Progress Tugas</Typography>
          <Typography variant={isMobile ? "caption" : "body2"} color="text.secondary" sx={{ mt: 0.5 }}>Perbandingan tugas selesai & pending</Typography>
        </Box>
        <Divider />
        <Box sx={{ p: { xs: 2, sm: 2.5, md: 3 } }}>
          {hasData && data?.overallStats?.total > 0 ? (
            <Stack direction={{ xs: "column", md: "row" }} sx={{ alignItems: "center", gap: { xs: 3, md: 4 } }}>
              ...
              <Stack sx={{ gap: { xs: 1.5, sm: 2 }, minWidth: { xs: "100%", md: 200 }, maxWidth: { xs: "100%", md: 280 }, width: "100%" }}>
                {[
                  { label: "Tugas Selesai", value: data.overallStats.completed || 0, color: theme.palette.secondary.main, bgAlpha: 0.06 },
                  { label: "Tugas Pending", value: data.overallStats.pending || 0, color: alpha(theme.palette.secondary.main, 0.6), bgAlpha: 0.04 },
                  { label: "Tingkat Penyelesaian", value: `${data.overallStats.completionRate || 0}%`, color: theme.palette.secondary.main, bgAlpha: 0.04 },
                ].map((item, i) => (
                  <Stack key={i} direction="row" sx={{ justifyContent: "space-between", alignItems: "center", p: { xs: 1.5, sm: 2 }, borderRadius: `${theme.shape.borderRadius}px`, bgcolor: alpha(theme.palette.secondary.main, item.bgAlpha) }}>
                    <Typography variant="body2" sx={{ fontWeight: 500 }}>{item.label}</Typography>
                    <Typography variant={isMobile ? "h6" : "h5"} sx={{ fontWeight: 700, color: item.color }}>{item.value}</Typography>
                  </Stack>
                ))}
              </Stack>
            </Stack>
          ) : (
            <EmptyState title="Belum Ada Data Tugas" description="Data tugas akan muncul di sini setelah Anda mulai mengerjakan"><EmptyMechanicSVG /></EmptyState>
          )}
        </Box>
      </Card>

      {/* Pending Tasks Table */}
      <AppTable title="Tugas Menunggu" subtitle={`${data?.pendingTasks?.length || 0} tugas perlu diselesaikan`} headers={tableHeaders} data={data?.pendingTasks || []} renderRow={renderRow} count={1} page={1} isLoading={isLoading} emptyStateMessage="Tidak ada tugas yang menunggu" enableMultiSelect={false} hideRowsPerPage />
    </Stack>
  );
};
...
MechanicDashboard.propTypes = {
  data: PropTypes.shape({
    overallStats: PropTypes.shape({ completed: PropTypes.number, pending: PropTypes.number, total: PropTypes.number, completionRate: PropTypes.number }),
    pendingTasks: PropTypes.arrayOf(PropTypes.shape({ assignmentId: PropTypes.string, orderItemId: PropTypes.string, orderId: PropTypes.string, orderNumber: PropTypes.string, plateNumber: PropTypes.string, serviceName: PropTypes.string, status: PropTypes.string })),
    todayTasks: PropTypes.shape({ completed: PropTypes.number, earnings: PropTypes.number, pending: PropTypes.number }),
  }),
