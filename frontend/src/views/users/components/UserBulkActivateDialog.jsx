...
import { useActivateUsersMutation } from "@views/users/hooks";
import { showNotification } from "@store/notifications/notificationsSlice.js";

const UserBulkActivateDialog = ({ selectedIds, selectedCount, onClose, onClearSelection, open }) => {
  const dispatch = useDispatch();

  const mutation = useActivateUsersMutation({
    onSuccess: (data) => {
      const { summary } = data;
      let message = `${summary.activated} karyawan berhasil diaktifkan`;
    ...
    onFailed: (error) => {
      dispatch(
        showNotification({
          message: error?.message || "Gagal mengaktifkan karyawan",
          type: "error",
          title: "Error",
          variant: "snackbar",
          autoHide: 5000,
    ...
    <Dialog fullWidth maxWidth="xs" onClose={mutation.isPending ? undefined : onClose} open={open}>
      ...
      <DialogContent sx={{ pt: 2.5, pb: 3 }}>
        ...
        <Typography variant="body2" color="success.main" sx={{ mt: 2 }}>
          Karyawan yang diaktifkan akan kembali dapat login dan mengakses sistem.
        </Typography>
      </DialogContent>
      ...
      <DialogActions sx={{ px: 3, py: 2.5 }}>
        ...
        >
          {mutation.isPending ? "Mengaktifkan..." : `Aktifkan ${selectedCount} Karyawan`}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
export default UserBulkActivateDialog;