...
async function seed() {
  await cleanDatabase();
  console.log("🌱 Mulai seeding database (180 PRODUK - ~300 ORDER/BULAN - JAN 2025 s/d MEI 2026)...\n");
  ...
  console.log("👥 [2/6] Membuat 300 Customers & Vehicles...");
  const dbCustomers = [];
  for (let i = 0; i < 300; i++) {
    const customer = await prisma.customer.create({
      data: { name: faker.person.fullName(), phone: faker.phone.number("08##########") },
    });
    ...
    for (let v = 0; v < vehicleCount; v++) {
      await prisma.vehicle.create({
        data: {
          ...
          brand: "Vespa", model: faker.helpers.arrayElement(vespaModels), customerId: customer.id,
        },
      });
    }
  }
  ...
  console.log(`🏍️  [3/6] Membuat Products (${sparepartData.length + serviceData.length})...`);
  const spareparts = [];
  const services = [];
  ...
  for (let i = 0; i < sparepartData.length; i++) {
    ...
    const initialStock = faker.number.int({ min: 300, max: 800 });
    const p = await prisma.product.create({
      data: {
        name: sparepartData[i].name, sku: generateSku("SPAREPART", i),
        type: "SPAREPART", description: sparepartData[i].description,
        price, cost, stock: initialStock, isActive: true,
      },
    });
    spareparts.push(p);
    await prisma.productPriceHistory.create({
      data: { productId: p.id, price: p.price, cost: p.cost, effectiveFrom: new Date("2025-01-01") },
    });
    await prisma.stockMovement.create({
      data: { productId: p.id, type: "IN", sourceType: "PURCHASE", quantity: initialStock, recordedById: admins[0].id, note: "Stok awal 2025", createdAt: new Date("2025-01-01") },
    });
  }
  ...
  for (let i = 0; i < serviceData.length; i++) {
    ...
    const p = await prisma.product.create({
      data: {
        name: serviceData[i].name, sku: generateSku("SERVICE", i),
        type: "SERVICE", description: serviceData[i].description,
  ...
  console.log("🕐 [4/6] Membuat Shifts & Expenses (Jan 2025 - Mei 2026)...\n");
  const shifts = [];
  ...
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
    ...
    const openedAt = new Date(d); openedAt.setHours(8, 0, 0, 0);
    const closedAt = new Date(d); closedAt.setHours(20, 0, 0, 0);
    const cashSales = faker.number.int({ min: 800000, max: 8000000 });
    const shift = await prisma.shift.create({
      data: {
        cashierId: cashier.id, status: "CLOSED", startingCash: 1000000,
        endingCash: 1000000 + cashSales, expectedCash: 1000000 + cashSales,
        cashSales, cashIn: 0, cashOut: 0, discrepancy: 0, openedAt, closedAt,
      },
    });
    shifts.push(shift);
  ...
  console.log("📋 [5/6] Membuat Orders (~300/bulan)...\n");
  let orderCount = 0;
  ...
  for (const monthKey of allMonths) {
    ...
    for (const sp of spareparts) {
      ...
      if (currentStock < 80) {
        const restockQty = faker.number.int({ min: 150, max: 400 });
        stockTracker[sp.id] = currentStock + restockQty;
        await prisma.product.update({ where: { id: sp.id }, data: { stock: { increment: restockQty } } });
        await prisma.stockMovement.create({
          data: { productId: sp.id, type: "IN", sourceType: "PURCHASE", quantity: restockQty, recordedById: admins[0].id, note: `Restock bulanan ${monthKey}`, createdAt: restockDate },
        });
      }
    }

    const adjustmentProducts = faker.helpers.arrayElements(spareparts, faker.number.int({ min: 5, max: 12 }));
    for (const sp of adjustmentProducts) {
      ...
      if (adjQty !== 0) {
        ...
        await prisma.stockMovement.create({
          data: { productId: sp.id, type: "ADJUSTMENT", sourceType: "ADJUSTMENT", quantity: Math.abs(adjQty), recordedById: admins[0].id, note: adjQty > 0 ? "Stock opname: kelebihan" : "Stock opname: kekurangan", createdAt: new Date(restockDate.getTime() + 3600000) },
        });
      }
    }

    const returnProducts = faker.helpers.arrayElements(spareparts, faker.number.int({ min: 1, max: 3 }));
    for (const sp of returnProducts) {
      const returnQty = faker.number.int({ min: 1, max: 5 });
      stockTracker[sp.id] += returnQty;
      await prisma.product.update({ where: { id: sp.id }, data: { stock: { increment: returnQty } } });
      await prisma.stockMovement.create({
        data: { productId: sp.id, type: "IN", sourceType: "RETURN", quantity: returnQty, recordedById: admins[0].id, note: "Retur pembelian sparepart", createdAt: new Date(restockDate.getTime() + 7200000) },
      });
    }

    const targetOrders = faker.number.int({ min: 290, max: 310 });
    ...
    for (const shift of shuffledShifts) {
      ...
      mechanics.forEach((m) => { if (!mechanicDailyTasks[m.id][shiftDay]) mechanicDailyTasks[m.id][shiftDay] = 0; });
      ...
      const orderStatuses = [
        ...Array(draftCount).fill("DRAFT"), ...Array(queuedCount).fill("QUEUED"),
        ...Array(inProgressCount).fill("IN_PROGRESS"), ...Array(completedCount).fill("COMPLETED"),
        ...Array(closedCount).fill("CLOSED"), ...Array(cancelledCount).fill("CANCELLED"),
      ];
      ...
      for (const status of orderStatuses) {
        ...
        switch (status) {
          case "DRAFT": break;
          case "QUEUED": diagnosedAt = new Date(baseDate); paymentMethod = faker.helpers.arrayElement(["CASH", "QRIS"]); break;
          case "IN_PROGRESS": diagnosedAt = new Date(baseDate); startedAt = new Date(baseDate.getTime() + 15 * 60000); paymentMethod = faker.helpers.arrayElement(["CASH", "QRIS"]); break;
          case "COMPLETED":
            diagnosedAt = new Date(baseDate); startedAt = new Date(baseDate.getTime() + 15 * 60000);
            completedAt = new Date(baseDate.getTime() + faker.number.int({ min: 30, max: 180 }) * 60000);
            paymentMethod = faker.helpers.arrayElement(["CASH", "QRIS"]); paymentStatus = "PAID"; paidAt = completedAt;
          ...
          case "CLOSED":
            ...
            paymentMethod = faker.helpers.arrayElement(["CASH", "QRIS"]); paymentStatus = "PAID"; paidAt = closedAt;
            amountPaid = paymentMethod === "CASH" ? total + faker.number.int({ min: 0, max: 200000 }) : total;
            changeAmount = paymentMethod === "CASH" ? amountPaid - total : 0;
            break;
          case "CANCELLED": deletedAt = new Date(baseDate.getTime() + 10 * 60000); break;
        }
        ...
        for (const item of selectedItems) {
          const orderItem = await prisma.orderItem.create({
            data: { orderId: order.id, productId: item.product.id, productNameSnapshot: item.product.name, quantity: item.qty, unitPrice: item.product.price, unitCostSnapshot: item.product.cost, subtotal: item.subtotal },
          });

          if (item.isService && assignedMechanicId && !["DRAFT", "CANCELLED"].includes(status)) {
            await prisma.mechanicAssignment.create({
              data: { orderItemId: orderItem.id, mechanicId: assignedMechanicId, startAt: ["QUEUED"].includes(status) ? null : startedAt, endAt: ["QUEUED", "IN_PROGRESS"].includes(status) ? null : completedAt },
            });
          }
          ...
          if (item.product.type === "SPAREPART" && ["IN_PROGRESS", "COMPLETED", "CLOSED"].includes(status)) {
            if (stockTracker[item.product.id] >= item.qty) {
              await prisma.stockMovement.create({
                data: { productId: item.product.id, type: "OUT", sourceType: "SALE", quantity: item.qty, orderItemId: orderItem.id, recordedById: shift.cashierId, createdAt: startedAt || diagnosedAt },
              });
        ...
        const notes = {
          DRAFT: ["Pesanan dibuat", "Order baru dicatat", "Pelanggan datang servis"],
        ...
        const statusFlow = [];
        statusFlow.push({ status: "DRAFT", note: faker.helpers.arrayElement(notes.DRAFT), changedById: shift.cashierId, createdAt: baseDate });
        if (["QUEUED", "IN_PROGRESS", "COMPLETED", "CLOSED"].includes(status)) {
          statusFlow.push({ status: "QUEUED", note: faker.helpers.arrayElement(notes.QUEUED), changedById: shift.cashierId, createdAt: diagnosedAt || new Date(baseDate.getTime() + 5 * 60000) });
        }
        if (["IN_PROGRESS", "COMPLETED", "CLOSED"].includes(status)) {
          const mechName = mechanics.find((m) => m.id === assignedMechanicId)?.fullName || "Mekanik";
          statusFlow.push({ status: "IN_PROGRESS", note: `${mechName} ${faker.helpers.arrayElement(notes.IN_PROGRESS)}`, changedById: assignedMechanicId || shift.cashierId, createdAt: startedAt || new Date(baseDate.getTime() + 20 * 60000) });
        }
        if (["COMPLETED", "CLOSED"].includes(status)) {
          statusFlow.push({ status: "COMPLETED", note: faker.helpers.arrayElement(notes.COMPLETED), changedById: assignedMechanicId || shift.cashierId, createdAt: completedAt || new Date(baseDate.getTime() + 60 * 60000) });
        }
        if (status === "CLOSED") {
          statusFlow.push({ status: "CLOSED", note: faker.helpers.arrayElement(notes.CLOSED), changedById: shift.cashierId, createdAt: closedAt || new Date(baseDate.getTime() + 90 * 60000) });
        }
        if (status === "CANCELLED") {
          statusFlow.push({ status: "CANCELLED", note: faker.helpers.arrayElement(notes.CANCELLED), changedById: shift.cashierId, createdAt: deletedAt || new Date(baseDate.getTime() + 10 * 60000) });
        }
  ...
  for (const user of createdUsers) {
    ...
    await prisma.notification.create({
      data: { title: "Selamat Bekerja", message: `Halo ${user.fullName}, sistem siap digunakan.`, type: "INFO", userId: user.id },
    });
  }
